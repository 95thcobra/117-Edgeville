		...
		private void processOutsidePlayers(Player player, RSBuffer buffer, RSBuffer appearanceUpdateBuffer, boolean b) {
			...
			for (int i = 0; i < player.sync().outsidePlayerIndexCount; i++) {
				...
				if (needsAdd(player, currentPlayer)) {
					...
					if (hash == player.sync().regionHashes[playerIndex])
						buffer.writeBits(1, 0);
					else {
						buffer.writeBits(1, 1);
						updateRegionHash(buffer, player.sync().regionHashes[playerIndex], hash);
						player.sync().regionHashes[playerIndex] = hash;
					}
					...
					buffer.writeBits(1, 1);
					localAddedPlayers++;
					player.sync().localPlayers[currentPlayer.index()] = currentPlayer;
					slotFlags[playerIndex] = (byte) (slotFlags[playerIndex] | 2);
				} else {
					int hash = currentPlayer == null ? player.sync().regionHashes[playerIndex]
							: currentPlayer.getTile().toRegionPacked();
					if (currentPlayer != null && hash != player.sync().regionHashes[playerIndex]) {
						buffer.writeBits(1, 1);
						updateRegionHash(buffer, player.sync().regionHashes[playerIndex], hash);
						player.sync().regionHashes[playerIndex] = hash;
					} else {
						buffer.writeBits(1, 0);
						for (int j = i + 1; j < player.sync().outsidePlayerIndexCount; j++) {
							int p2Index = player.sync().outsidePlayerIndexes[j];
							if (b ? slotFlags[p2Index] == 0 : slotFlags[p2Index] != 0)
								continue;

							Player p2 = player.world().players().get(p2Index);
							if (needsAdd(player, p2)
									|| (p2 != null && p2.getTile().toRegionPacked() != player.sync().regionHashes[p2Index]))
								break;
							skip++;
		...
		private void processLocalPlayers(Player player, RSBuffer buffer, RSBuffer appearanceUpdateBuffer, boolean b) {
			...
			for (int i = 0; i < player.sync().localPlayerIndexCount; i++) {
				...
				if (needsRemove(player, currentPlayer)) {
					...
					player.sync().regionHashes[playerIndex] = currentPlayer.activeMap() == null
					...
					player.sync().localPlayers[playerIndex] = null;
				} else {
					...
					if (currentPlayer.sync().teleported()) {
						...
						for (int j = i + 1; j < currentPlayer.sync().localPlayerPtr(); j++) {
							...
							if (b ? currentPlayer.sync().calculatedFlag() != 0
								...
								continue;
							Player p2 = player.sync().localPlayers[player2Index];
